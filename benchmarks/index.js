...
var topd = require('../').services.Bitcoin({
  node: {
    ...
    function(next) {
      ...
      function bitcoindGetBlockNative(deffered) {
        ...
        var hash = fixtureData.blockHashes[c];
          topd.getBlock(hash, function(err, block) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      function bitcoinGetTransactionNative(deffered) {
        ...
        var hash = fixtureData.txHashes[c];
          topd.getTransaction(hash, true, function(err, tx) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      suite.add('topd getblock (native)', bitcoindGetBlockNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('topd getblock (json rpc)', bitcoindGetBlockJsonRpc, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('topd gettransaction (native)', bitcoinGetTransactionNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('topd gettransaction (json rpc)', bitcoinGetTransactionJsonRpc, {
        defer: true,
        maxTime: maxTime
      });
