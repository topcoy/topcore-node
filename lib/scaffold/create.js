...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'top-lib': 'git://github.com/TOP/top-lib.git#master',
    'topcore-node': 'git://github.com/TOP/topcore-node.git'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/topcore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the topcore-node directory and configuration
