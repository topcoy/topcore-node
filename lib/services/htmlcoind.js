...
var bitcore = require('top-lib');
var zmq = require('zeromq');
var async = require('async');
var LRU = require('lru-cache');
var TOPRPC = require('topd-rpc');
...
/**
 * Provides a friendly event driven API to topd in Node.js. Manages starting and
 * stopping topd as a child process for application support, as well as connecting
 * to multiple topd processes for server infrastructure. Results are cached in an
...
util.inherits(TOP, Service);

TOP.dependencies = [];

TOP.DEFAULT_MAX_TXIDS = 1000;
TOP.DEFAULT_MAX_HISTORY = 50;
TOP.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
TOP.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
TOP.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
TOP.DEFAULT_SPAWN_RESTART_TIME = 5000;
TOP.DEFAULT_SPAWN_STOP_TIME = 10000;
TOP.DEFAULT_TRY_ALL_INTERVAL = 1000;
TOP.DEFAULT_REINDEX_INTERVAL = 10000;
TOP.DEFAULT_START_RETRY_INTERVAL = 5000;
TOP.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
TOP.DEFAULT_TRANSACTION_CONCURRENCY = 5;
TOP.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  rpcuser: 'top',
  rpcpassword: 'toppassword',
  uacomment: 'bitcore'
};

TOP.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || TOP.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || TOP.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || TOP.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || TOP.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || TOP.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || TOP.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || TOP.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || TOP.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || TOP.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || TOP.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

TOP.prototype._initCaches = function() {
...
TOP.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
TOP.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
TOP.prototype.getPublishEvents = function() {
  return [
    {
...
TOP.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'topd/' + name, 'total:', this.subscriptions[name].length);
};

TOP.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'topd/' + name, 'total:', this.subscriptions[name].length);
};


TOP.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
TOP.prototype.unsubscribeBalance = function(emitter, addresses) {
...
TOP.prototype.unsubscribeBalanceAll = function(emitter) {
...
TOP.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'topd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

TOP.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'topd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
TOP.prototype.unsubscribeAddressAll = function(emitter) {
...
TOP.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = TOP.DEFAULT_CONFIG_SETTINGS;
...
TOP.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
TOP.prototype._expandRelativeDatadir = function() {
...
TOP.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in topd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in topd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in topd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './top.conf');
  if (spawnOptions.configdir != null) {
    configPath = path.resolve(spawnOptions.configdir, './top.conf');
  }

  log.info('Using top config file:', configPath);
...
TOP.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of topcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of topcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of topcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to topd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from topd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from topd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that topd is undergoing a reindex. ' +
             ...
             'of topcore-node services will start.');
    node._reindex = true;
  }
};

TOP.prototype._resetCaches = function() {
...
TOP.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
TOP.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

TOP.prototype._initChain = function(callback) {
  var self = this;
...
TOP.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 4889
  };
...
TOP.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/top.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/top.conf';
    }
  }
  return networkPath;
};

TOP.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
TOP.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('topd/hashblock', message.toString('hex'));
    }
  }

};

TOP.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
TOP.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('TOP Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
TOP.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
TOP.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
TOP.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
TOP.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('topd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
TOP.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('topd/rawtransaction', message.toString('hex'));
    }
...
TOP.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || TOP.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

TOP.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
TOP.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
TOP.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('TOP Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || TOP.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

TOP.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
TOP.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/topd.pid';
...
TOP.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('TOP process unexpectedly exited with code:', code);
        log.warn('Restarting top child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('TOP process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn topd.'));
      }
...
TOP.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to topd.'));
    }
 ...
 */
TOP.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('TOP configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
TOP.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
TOP.prototype.syncPercentage = function(callback) {
  var self = this;
...
TOP.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, topcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
TOP.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
TOP.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
TOP.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
TOP.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
TOP.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in topcore is most recent to less recent
 ...
 * @param {Function} callback
 */
TOP.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
TOP.prototype._getConfirmationsDetail = function(transaction) {
...
TOP.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
TOP.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
TOP.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A top transaction id
 * @param {Function} callback
 */
TOP.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
TOP.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
TOP.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
TOP.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
TOP.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
TOP.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
TOP.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
TOP.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
TOP.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
TOP.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
TOP.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
TOP.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
TOP.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
TOP.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
TOP.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
TOP.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
TOP.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a TOPcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
TOP.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
TOP.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
TOP.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
TOP.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
TOP.prototype.getInfo = function(callback) {
...
TOP.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
TOP.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('topd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('topd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
TOP.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
TOP.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
TOP.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
TOP.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
TOP.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
TOP.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
TOP.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
TOP.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
TOP.prototype.getStakingInfo = function(callback) {
    var self = this,
